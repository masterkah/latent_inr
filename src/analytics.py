...
def visualize_grid_perturbation(
    model, dataset, img_idx, points_to_perturb, epsilon, device, run_folder
):
    """
    ...
    """
    print(
        f"Running Spatial Perturbation Analysis on Image {img_idx} with epsilon={epsilon}..."
    )
    ...
    for i, (py, px) in enumerate(points_to_perturb):
        ...
        pert_flat = perturb_latent_and_reconstruct(
            model, img_idx, dataset, (py, px), epsilon, device
        )
    ...
    save_path = os.path.join(
        run_folder, f"perturb_analysis_img_{img_idx}_eps{epsilon}.png"
    )
    plt.savefig(save_path)
    plt.close()
    _restore_model_mode(model, was_training)
